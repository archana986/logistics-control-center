...
import { listCatalogs, listSchemas, listTables, getTableColumns, getConfig, updateConfig } from "@/lib/api-client";
...
export const Route = createFileRoute("/_sidebar/configs_/$configId")({
  component: EditConfigWrapper,
});

function EditConfigWrapper() {
  return (
    <Suspense fallback={<EditConfigSkeleton />}>
      <EditConfig />
    </Suspense>
  );
}
...
function EditConfig() {
  ...
  const [sourceCatalog, setSourceCatalog] = useState(config.source_catalog);
  const [sourceSchema, setSourceSchema] = useState(config.source_schema);
  const [workersTable, setWorkersTable] = useState(config.workers_table);
  const [shiftsTable, setShiftsTable] = useState(config.shifts_table);
  const [availabilityTable, setAvailabilityTable] = useState(config.availability_table);
  
  // Column mappings
  const [workerNameCol, setWorkerNameCol] = useState(config.worker_name_col);
  const [workerPayCol, setWorkerPayCol] = useState(config.worker_pay_col);
  const [shiftNameCol, setShiftNameCol] = useState(config.shift_name_col);
  const [shiftRequiredCol, setShiftRequiredCol] = useState(config.shift_required_col);
  const [availabilityWorkerCol, setAvailabilityWorkerCol] = useState(config.availability_worker_col);
  const [availabilityShiftCol, setAvailabilityShiftCol] = useState(config.availability_shift_col);
  
  // Optimization settings
  const [maxShiftsPerWorker, setMaxShiftsPerWorker] = useState<string>(
    config.max_shifts_per_worker?.toString() || ""
  );
  const [timeLimitSeconds, setTimeLimitSeconds] = useState(
    config.time_limit_seconds?.toString() || "60"
  );
  
  // Target settings
  const [targetCatalog, setTargetCatalog] = useState(config.target_catalog || "");
  const [targetSchema, setTargetSchema] = useState(config.target_schema || "");
  const [resultsTable, setResultsTable] = useState(config.results_table || "");
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Parameters<typeof updateConfig>[1]) => updateConfig(configId, data),
    onSuccess: () => {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Update your workforce optimization configuration
          </p>
        </div>
      </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={workerNameCol} onValueChange={setWorkerNameCol}>
                    ...
                    <SelectContent>
                      ...
                      {!workersColumns && (
                        <SelectItem value={workerNameCol}>{workerNameCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={workerPayCol} onValueChange={setWorkerPayCol}>
                    ...
                    <SelectContent>
                      ...
                      {!workersColumns && (
                        <SelectItem value={workerPayCol}>{workerPayCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={shiftNameCol} onValueChange={setShiftNameCol}>
                    ...
                    <SelectContent>
                      ...
                      {!shiftsColumns && (
                        <SelectItem value={shiftNameCol}>{shiftNameCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={shiftRequiredCol} onValueChange={setShiftRequiredCol}>
                    ...
                    <SelectContent>
                      ...
                      {!shiftsColumns && (
                        <SelectItem value={shiftRequiredCol}>{shiftRequiredCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={availabilityWorkerCol} onValueChange={setAvailabilityWorkerCol}>
                    ...
                    <SelectContent>
                      ...
                      {!availabilityColumns && (
                        <SelectItem value={availabilityWorkerCol}>{availabilityWorkerCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={availabilityShiftCol} onValueChange={setAvailabilityShiftCol}>
                    ...
                    <SelectContent>
                      ...
                      {!availabilityColumns && (
                        <SelectItem value={availabilityShiftCol}>{availabilityShiftCol}</SelectItem>
                      )}
                    </SelectContent>
                  </Select>
                </div>
