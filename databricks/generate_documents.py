"""Generate unstructured documents for Knowledge Assistant.

Creates markdown documents including:
...
Writes documents to UC Volume for Knowledge Assistant indexing.
"""
...
def write_document_to_volume(client: WorkspaceClient, volume_path: str, filename: str, content: str):
    """Write a document to UC Volume."""
    try:
        # Use Files API to write to volume
        full_path = f"/Volumes/{volume_path}/{filename}"
        client.files.upload(full_path, content.encode('utf-8'))
        print(f"  ✓ Wrote {filename}")
    except Exception as e:
        print(f"  ✗ Error writing {filename}: {e}")


def generate_incident_reports() -> list[tuple[str, str]]:
...
def generate_maintenance_bulletins() -> list[tuple[str, str]]:
...
def generate_operational_procedures() -> list[tuple[str, str]]:
...
def generate_sla_documents() -> list[tuple[str, str]]:
...
def generate_route_guides() -> list[tuple[str, str]]:
...
def generate_rca_reports() -> list[tuple[str, str]]:
...
def generate_all_documents(client: WorkspaceClient, volume_path: str):
    """Generate all document types and write to UC Volume."""
    print("Generating documents for Knowledge Assistant...\n")
    ...
    # Write all documents
    print(f"Writing {len(all_docs)} documents to {volume_path}...\n")
    for filename, content in all_docs:
        write_document_to_volume(client, volume_path, filename, content)
...
def main():
    ...
    generate_all_documents(client, volume_path)
